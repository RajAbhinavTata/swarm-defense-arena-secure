...
import { buildRedTeamFeedItems, buildTaskAgentFeedItems } from '@/lib/run-engine/duel-feed';
import { formatDateTime, formatDuration } from '@/lib/run-engine/format';
import type { ArenaSession } from '@/lib/run-engine/types';
...
export function FinisherClient({ gameId }: { gameId: string }) {
  ...
  useEffect(() => {
    void fetch(`/api/run-control/${gameId}`, { cache: 'no-store' })
    ...
    <div className="arena-ui-shell">
      ...
      <main className="fin-main">
        ...
        <div className="fin-actions">
          <Link href="/launch"   className="fin-btn-primary">New Run →</Link>
          <Link href="/run-log"     className="fin-btn-secondary">Archive</Link>
          <a href={`/api/run-control/${gameId}/artifact?format=json`}     className="fin-btn-secondary">JSON</a>
          <a href={`/api/run-control/${gameId}/artifact?format=csv`}      className="fin-btn-secondary">CSV</a>
          <a href={`/api/run-control/${gameId}/artifact?format=sharegpt`} className="fin-btn-secondary">ShareGPT</a>
        </div>
