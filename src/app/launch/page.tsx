...
import { LIVE_WEB_PRESETS } from '@/lib/run-engine/live-web-presets';
import type { Difficulty, RedTeamType, TaskAgentType } from '@/lib/run-engine/types';
...
export default function ConfigurePage() {
  ...
  async function launch() {
    ...
    try {
      const res = await fetch('/api/run-control/launch', {
        method: 'POST',
          ...
          <div className="fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.625rem', marginBottom: '1rem' }}>
                {LIVE_WEB_PRESETS.map((p) => (
                  <button
                    key={p.id}
                    type="button"
                    onClick={() => { setPresetId(p.id); setTargetUrl(p.url); setTask(p.task); setUrlError(''); }}
                    style={{
                      ...presetBtnStyle,
            ...
            <div style={{ marginBottom: '0.5rem' }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--tx3)', marginBottom: '0.5rem' }}>
                Write this in plain English. Example: &ldquo;Search for blue running shoes and add the cheapest one to the cart.&rdquo;
              </p>
              <textarea
                id="task"
                value={task}
