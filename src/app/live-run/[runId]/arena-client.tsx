...
import { SwarmArenaShell } from '@/components/operations-console/swarm-arena-shell';
import { OperationsHeader } from '@/components/operations-header';
import { buildViewportAlertState, buildSwarmTimeline } from '@/lib/threat-intelligence/attack-visualization';
import { buildThreatMetrics } from '@/lib/threat-intelligence/threat-metrics';
...
export function ArenaClient({ gameId }: { gameId: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const response = await fetch(`/api/run-control/${gameId}`, { cache: 'no-store' });
        if (!response.ok) {
