...
import { formatDateTime, formatDuration, formatPercent } from '@/lib/run-engine/format';
import { SCENARIOS } from '@/lib/run-engine/scenarios';
import type { AttackFamily, Difficulty, FinalVerdict, ArenaSession } from '@/lib/run-engine/types';
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    void fetch('/api/run-control/catalog', { cache: 'no-store' })
      .then((r) => r.json())
        ...
        <div style={{
          ...
          {filtered.length === 0 ? (
            <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--tx3)', fontSize: 14 }}>
              No runs found.{' '}
              <Link href="/launch" style={{ color: 'var(--accent)', fontWeight: 600 }}>Start a test</Link>
              {' '}to see results here.
            </div>
          ) : (
                    ...
                    return (
                      <tr key={s.gameId} style={{ borderBottom: i < filtered.length - 1 ? '1px solid var(--border)' : 'none' }}>
                        ...
                        <td style={{ padding: '0.75rem 1rem' }}>
                          <Link href={`/run-log/${s.gameId}`} style={{ color: 'var(--accent)', fontWeight: 500 }}>
                            {s.scenarioLabel}
                          </Link>
                        </td>
                        <td style={{ padding: '0.75rem 1rem' }}>
