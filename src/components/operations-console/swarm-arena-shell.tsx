...
import { LiveViewportFrame } from '@/components/operations-console/live-viewport-frame';
import { SwarmLegend } from '@/components/operations-console/swarm-legend';
import { SwarmTopHud } from '@/components/operations-console/swarm-top-hud';
import { ThreatPanel } from '@/components/operations-console/threat-panel';
import type { ViewportAlertState, SwarmTimelineEntry } from '@/lib/threat-intelligence/attack-visualization';
import type { SwarmArenaMetrics } from '@/lib/threat-intelligence/threat-metrics';
import type { DuelFeedItem } from '@/lib/run-engine/duel-feed';
import type { RedTeamAction, ArenaSession, TaskAgentStep } from '@/lib/run-engine/types';
        ...
        <article className="card swarm-log-card">
          <div className="swarm-card-head">
            ...
            <div className="swarm-export-links">
              <a href={`/api/run-control/${gameId}/artifact?format=json`} className="chip chip-accent">
                JSON
              </a>
              <a href={`/api/run-control/${gameId}/artifact?format=csv`} className="chip chip-accent">
                CSV
              </a>
              <a href={`/api/run-control/${gameId}/artifact?format=sharegpt`} className="chip chip-accent">
                ShareGPT
              </a>
              {session.endedAt ? (
                <Link href={`/report/${gameId}`} className="chip chip-task">
                  Finisher
                </Link>
              ) : null}
            </div>
