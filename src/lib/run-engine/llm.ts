...
export function getTaskAgentLlmConfig(): LlmRuntimeConfig | null {
  return buildConfig({
    providerEnv: process.env.ARENA_TASK_AGENT_PROVIDER,
    modelEnv: process.env.ARENA_TASK_AGENT_MODEL,
    temperatureEnv: process.env.ARENA_TASK_AGENT_TEMPERATURE,
...
export function getRedTeamLlmConfig(): LlmRuntimeConfig | null {
  const preferred = buildConfig({
    providerEnv: process.env.ARENA_RED_TEAM_PROVIDER,
    modelEnv: process.env.ARENA_RED_TEAM_MODEL,
    temperatureEnv: process.env.ARENA_RED_TEAM_TEMPERATURE,
  ...
  return {
    provider: 'openai',
    model: (process.env.ARENA_RED_TEAM_OPENAI_MODEL?.trim() || 'gpt-5-mini').trim(),
    temperature: parseTemperature(process.env.ARENA_RED_TEAM_TEMPERATURE, 0.7),
    apiKey: openAiKey,
  };
}
...
async function callOpenAiJson<T>(
  ...
  if (process.env.ARENA_LLM_DEBUG === '1' && lastError) {
    console.warn(`[arena][llm][openai] model=${config.model} body=${lastError.slice(0, 400)}`);
  }
  return null;
}
...
async function callAnthropicJson<T>(
  ...
  if (process.env.ARENA_LLM_DEBUG === '1' && lastError) {
    console.warn(`[arena][llm][anthropic] model=${config.model} body=${lastError.slice(0, 400)}`);
  }
  return null;
}
...
export async function callLlmJson<T>(params: {
  ...
  const timeoutMs = params.timeoutMs ?? Number(process.env.ARENA_LLM_TIMEOUT_MS || DEFAULT_TIMEOUT_MS);
