...
export async function extractDomSummary(page: Page): Promise<DomSummary> {
  return page
    ...
    .then((summary) => {
      ...
      return {
        ...summary,
        visibleText: summary.visibleText.map((value) => redactSensitiveText(value) ?? ''),
        buttonLabels: summary.buttonLabels.map((value) => redactSensitiveText(value) ?? ''),
        actionableElements: summary.actionableElements.map((entry) => ({
          ...entry,
          label: redactSensitiveText(entry.label) ?? '',
        })),
        suspiciousSignals,
      };
    });
