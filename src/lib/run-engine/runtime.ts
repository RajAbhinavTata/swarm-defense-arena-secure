...
declare global {
  var __arena_runtime__: RuntimeState | undefined;
}

function getState(): RuntimeState {
  globalThis.__arena_runtime__ ??= { active: new Map(), queue: [] };
  return globalThis.__arena_runtime__;
}

function concurrency(): number {
  const parsed = Number(process.env.ARENA_MAX_CONCURRENT_RUNS ?? 2);
