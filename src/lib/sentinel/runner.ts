...
export async function runSimulation(config: RunnerConfig): Promise<void> {
  ...
  try {
    ...
    await page.goto(initialUrl, {
      ...
      timeout: Math.max(5_000, Number(process.env.SENTINEL_NAVIGATION_TIMEOUT_MS ?? 30_000)),
    });
    ...
    const captureScreenshot = async (
      ...
      try {
        ...
        const screenshotUrl = `/api/sentinel/${gameId}/screens/${path.basename(screenshotPath)}`;
