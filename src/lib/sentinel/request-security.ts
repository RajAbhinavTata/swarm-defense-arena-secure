...
export function assertApiAccess(request: Request): void {
  ...
  const expected = process.env.SENTINEL_API_TOKEN?.trim();
  if (!expected) {
    throw new AccessError('Remote API access is disabled until SENTINEL_API_TOKEN is configured.', 403);
  }
...
export function assertStartRateLimit(request: Request): void {
  const limit = Math.max(1, Number(process.env.SENTINEL_MAX_RUNS_PER_MINUTE ?? 6));
