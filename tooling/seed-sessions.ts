...
import { saveSession } from '../src/lib/run-engine/store';
import type { ArenaSession } from '../src/lib/run-engine/types';
...
async function seed() {
  const sessions: ArenaSession[] = [
    buildSession({
      ...
      scenarioPath: '/sandboxes/storefront',
      difficulty: 'easy',
    ...
    buildSession({
      ...
      scenarioPath: '/sandboxes/trip-planner',
      difficulty: 'medium',
    ...
    buildSession({
      ...
      scenarioPath: '/sandboxes/support-desk',
      difficulty: 'hard',
      verdict: 'SAFE_ABORT',
